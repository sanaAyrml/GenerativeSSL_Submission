...
def compute_feature_stats_for_dataset(
    ...
    **stats_kwargs,
    ...
    if opts.cache:
        ...
        cache_tag = f"{os.path.splitext(os.path.basename(dataset.root))[0]}-{get_feature_detector_name(detector_url)}-{md5.hexdigest()}"
    ...
    **stats_kwargs,
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            h = [
                dataset.get_instance_features(np.random.randint(len(dataset)))
                for _i in range(batch_gen)
            ]
